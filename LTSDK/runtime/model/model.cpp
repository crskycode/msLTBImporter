...
{
	uint32 iObjSize;
	file.Read(&iObjSize,sizeof(iObjSize));
	uint32 iPos;
	file.GetPos(&iPos);

	uint32 dummy;

	uint32 iVertCount, iUnDupVertCount, iPolyCount, iMaxBonesPerTri, iMaxBonesPerVert;
	uint32 iBone;
	uint8 bReIndexBones;
	uint8 bUseMatrixPalettes;
	...
	// eRigidMesh
	//-------------------------------------------------------
	if (m_iObjectType == eRigidMesh)
	{
		file >> iVertCount;
		file >> iPolyCount;
		file >> iMaxBonesPerTri;
		file >> iMaxBonesPerVert;
		file.Read(StreamData, sizeof(uint32)*4);

		file >> iBone;

		m_Verts.SetSize(iVertCount);
		m_Tris.SetSize(iPolyCount);

		for (uint32 iStream = 0; iStream < 1; ++iStream)
		{
			for (uint32 i = 0; i < iVertCount; ++i)
			{
				memset(&v, 0, sizeof(ModelVert));
	...
	// eSkelMesh
	//-------------------------------------------------------
	else if (m_iObjectType == eSkelMesh)
	{
		file >> iVertCount;
		file >> iPolyCount;
		file >> iMaxBonesPerTri;
		...
		file >> bReIndexBones;
		file.Read(StreamData, sizeof(uint32)*4);
		file >> bUseMatrixPalettes;

		if (!bUseMatrixPalettes)
			Load_SkelMesh_RenderDirect(file, iVertCount, iPolyCount, iMaxBonesPerTri, iMaxBonesPerVert, bReIndexBones, StreamData, bUseMatrixPalettes);
		else
			Load_SkelMesh_MatrixPalettes(file, iVertCount, iPolyCount, iMaxBonesPerTri, iMaxBonesPerVert, bReIndexBones, StreamData, bUseMatrixPalettes);
	}
	//-------------------------------------------------------
	// eVAMesh
	//-------------------------------------------------------
	else if (m_iObjectType == eVAMesh)
	{
		file >> iVertCount;
		file >> iUnDupVertCount;
		file >> iPolyCount;
		file >> iMaxBonesPerTri;
		file >> iMaxBonesPerVert;
		file.Read(StreamData, sizeof(uint32)*4);
		file >> dummy;		// m_vaAnimNodeIdx
		file >> dummy;		// it's always 0 for VA models for now - we'll probably change this in spring
...
bool CDIModelDrawable::Load_SkelMesh_MatrixPalettes(ILTStream &file, uint32 iVertCount, uint32 iPolyCount, uint32 iMaxBonesPerTri, uint32 iMaxBonesPerVert, uint8 bReIndexBones, uint32 (&StreamData)[4], uint8 bUseMatrixPalettes)
{
	uint32 dummy;

	uint32 iMinBone, iMaxBone;
	uint32 iReIndexedBones;

	ModelVert v;
	ModelTri t;

	CMoArray<int, NoCache> ReIndexedBoneMap;
	...
	if (bReIndexBones)
	{
		file >> iReIndexedBones;

		ReIndexedBoneMap.SetSize(iReIndexedBones);

		for (uint32 i = 0; i < iReIndexedBones; i++)
		{
			file >> dummy;
			ReIndexedBoneMap[i] = dummy;
		}
	}
		...
		for (uint32 i = 0; i < iVertCount; ++i)
		{
			...
			if (StreamData[iStream] & VERTDATATYPE_POSITION)
			{
				...
				if (bReIndexBones)
				{
					for (uint32 j = 0; j < 4; ++j)
					{
						v.m_Weights.m_iBone[j] = v.m_Weights.m_iBone[j] ? ReIndexedBoneMap[ v.m_Weights.m_iBone[j] ] : 0;
					}
				}
			}
